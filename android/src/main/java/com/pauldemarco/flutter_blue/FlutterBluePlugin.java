...
public class FlutterBluePlugin implements FlutterPlugin, ActivityAware, MethodCallHandler, RequestPermissionsResultListener  {
    ...
    public void onMethodCall(MethodCall call, Result result) {
        ...
        switch (call.method) {
            ...
            case "startScan":
            {
                //check if is android 10 or above
                    ...
                    if (ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_BACKGROUND_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                        ...
                        if (activity.shouldShowRequestPermissionRationale(Manifest.permission.ACCESS_BACKGROUND_LOCATION)) {
                            new AlertDialog.Builder(activity)
                                    ...
                                    .setMessage("Background Location permission is needed to scan for devices")
                                    .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                                        @Override
                                        public void onClick(DialogInterface dialog, int which) {
                                            activityBinding.getActivity().requestPermissions(new String[]{Manifest.permission.ACCESS_BACKGROUND_LOCATION}, REQUEST_FINE_LOCATION_PERMISSIONS);
                                        }
                                    })
                                    .setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
                                        @Override
                                        public void onClick(DialogInterface dialog, int which) {
                                            Log.i(TAG, "onClick: permission denied");
//                                            result.error("permission_denied", "Location permission is needed to scan for devices", null);
                                        }
                                    })
                                    .show();
                        } else {
    ...
    public boolean onRequestPermissionsResult(
        ...
        if (requestCode == REQUEST_FINE_LOCATION_PERMISSIONS) {
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                if (permissions.length == 1 && permissions[0].equals(Manifest.permission.ACCESS_BACKGROUND_LOCATION)) {
                    //Check if location services are enabled
                    LocationManager locationManager = (LocationManager) context.getSystemService(Context.LOCATION_SERVICE);
                    if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
                        //We have the permissions and location services are enabled, so we can start scanning
                        startScan(pendingCall, pendingResult);
                    } else {
                        //show a dialog to the user to enable location services
                        AlertDialog.Builder builder = new AlertDialog.Builder(context);
                        builder.setTitle("Location Services Not Enabled");
                        builder.setMessage("Please enable location services to scan for devices");
                        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                //Enable location services dialog
    ...
    private void startScan(MethodCall call, Result result) {
        ...
        try {
            ...
            android.util.Log.e(TAG, "startScan: "+e.getMessage() );
            try {
                result.error("startScan", e.getMessage(), e.getCause());
            } catch (Exception e1) {
                android.util.Log.e(TAG, "startScan: "+e1.getMessage() );
            }
        }
    }
    ...
    private void startScan21(Protos.ScanSettings proto) throws IllegalStateException {
        Log.i(TAG, "startScan21: ");
        //check if android 10 and above
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            LocationManager locationManager = (LocationManager) context.getSystemService(Context.LOCATION_SERVICE);
            if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
                Log.i(TAG, "startScan21: GPS is enabled");

            } else {
                Log.i(TAG, "startScan21: GPS is disabled");
                //show a dialog to the user to enable location services
                AlertDialog.Builder builder = new AlertDialog.Builder(activityBinding.getActivity());
                builder.setTitle("Location Services Not Enabled");
                builder.setMessage("Please enable location services to scan for devices");
                builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    @Override
        ...
        android.util.Log.e(TAG, "startScan21: start abdroid scan:" );
